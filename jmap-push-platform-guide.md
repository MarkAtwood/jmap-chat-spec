...
payload is delivered to the app silently. Always use data messages. With a display
message, the OS generates a notification before your app runs — you cannot apply
JMAP-specific logic (mention highlighting, E2E decryption, deduplication) to a
notification that has already been shown.
...
On OAuth token refresh failure or APNs JWT signing failure, retry with exponential
backoff. Do not destroy the `PushSubscription` — a credential problem means your server
cannot reach the platform right now; it says nothing about whether the device registration
is still valid. Destroy the subscription only when the platform explicitly tells you the
token or endpoint is gone (e.g. FCM `UNREGISTERED`, APNs `Unregistered`, HTTP `404` or
`410`).
...
hour; cache and refresh proactively.
...
`data` values must be strings; embed your payload JSON as an escaped string under an
application-defined key (`"jmap-push"` is a reasonable choice). Omit `"notification"`.

**iOS via FCM:** Replace the `"android"` block with an `"apns"` block. FCM can only
deliver to iOS if an APNs auth key (`.p8`) or certificate is configured in the Firebase
project — without it, FCM accepts the request but silently drops delivery.
...
`"HIGH"` wakes devices in Doze mode. `"NORMAL"` is opportunistic. FCM has no lower tier;
map `"low"` and `"very-low"` to `"NORMAL"`.

### Error responses

FCM returns `200 OK` even when delivery fails — always check the response body:

- `error.status: "UNREGISTERED"` or `"SENDER_ID_MISMATCH"` — destroy the `PushSubscription`
- `error.status: "QUOTA_EXCEEDED"` — back off and retry
- `error.status: "INVALID_ARGUMENT"` — payload problem; log and fix, do not retry
...
Tokens are valid for one hour; cache and refresh proactively.
...
`data` values must be strings. Set `expiresAfter` to reflect how long the notification is
still useful; use shorter values for higher urgency.
...
ADM returns a JSON body with a `reason` field. Parse it — HTTP status alone is not
sufficient:

- `InvalidRegistrationId` / `Unregistered` — destroy the `PushSubscription`
- `MaxRateExceeded` — back off and retry
- `MessageTooLarge` — reduce payload and retry
...
`"data"` is a single string (not a map); serialize your entire payload JSON into it. Omit
`"notification"`.
...
HPK returns a JSON body with a `code` field on every response regardless of HTTP status.
Success is `code: 80000000`. Parse the `code` field to classify errors — HTTP status
...
MiPush uses a static App Secret from the Xiaomi Developer Console. Include it on every
request — no token exchange, no expiry:
...
`payload` is a string; URL-encode your JSON payload and place it there. `pass_through=1`
routes to the app's broadcast receiver silently. `time_to_live` is in milliseconds.
...
Check current Xiaomi developer documentation for per-message priority fields that may be
available in newer API versions.

### Error responses

MiPush returns a JSON body. `result: "ok"` on success; on failure, the `code` field
identifies the error. Consult current Xiaomi developer documentation for the full code
reference. Invalid or unregistered regIds indicate the subscription should be destroyed;
rate-limit errors call for backoff and retry.
...
If targeting mainland China, support MiPush and HPK at minimum; OPPO and Vivo add
coverage depending on your target market.
...
APNs has two delivery modes; choose based on your urgency requirements:
...
constructs and posts the notification. Delivered opportunistically — not suitable for
high-urgency notifications.

**Alert push** (`apns-push-type: alert`, `apns-priority: 10`, with an `"alert"` key):
the OS can display a notification immediately. Use a
...
Set `apns-expiration` to limit how long APNs queues an undelivered notification; omitting
it queues indefinitely. Shorter expiries suit higher urgency (e.g. 1 hour for `"high"`,
24 hours for `"normal"`).
...
Background push cannot guarantee timely delivery regardless of urgency. If you need
prompt delivery for high-urgency notifications, use alert mode.

### Error responses

APNs returns a JSON body with a `reason` field on 4xx/5xx responses. HTTP status alone is
not sufficient — parse `reason`:

- `BadDeviceToken` / `DeviceTokenNotForTopic` — destroy the `PushSubscription`
- `Unregistered` / `ExpiredToken` — destroy the `PushSubscription`
- `TooManyRequests` — back off and retry
- `BadTopic` — wrong `apns-topic` value; configuration error, not a subscription error
...
Store all three. The server needs `p256dh` and `auth` to encrypt each push.

### Auth (VAPID)

Generate a P-256 ECDSA key pair once at setup and persist the private key. Include on
every request:
...
JWT claims: `aud` (push service origin, e.g. `https://push.example.com`), `sub` (your
contact URI, e.g. `mailto:admin@example.com`), `exp` (at most 24 hours from now).

### Payload encryption

Encrypt your payload bytes per RFC 8291 (`aes128gcm`) using the client's `p256dh` and
`auth` values. Libraries: `web-push` (Node.js), `pywebpush` (Python), `web-push` crate
(Rust), `webpush-go` (Go).
...
Set `TTL` to reflect how stale a notification can be before it becomes noise (e.g. 3600
for `"high"`, 86400 for `"normal"`).

### Urgency

Web Push urgency values are identical to JMAP's. Pass them through directly — no
translation needed:
...
16-byte AES-GCM authentication tag, and a 1-byte padding delimiter. Keep plaintext under
**3993 bytes** to stay within the 4096-byte ciphertext limit.

### ntfy and UnifiedPush

ntfy exposes standard Web Push endpoints — treat it identically to any other Web Push
delivery, no special casing required.

UnifiedPush routes through user-chosen distributors (ntfy, Gotify, a self-hosted relay,
etc.). Treat any UnifiedPush endpoint identically to Web Push. RFC 8291 encryption
support varies by distributor. Test with an encrypted payload against each distributor
you plan to support; fall back to unencrypted delivery for distributors that reject
encrypted payloads. Note that unencrypted delivery exposes payload content to the
...
Verify this endpoint against current Microsoft WNS documentation — Microsoft has been
migrating authentication infrastructure. Tokens are valid for 24 hours; cache and refresh
proactively.
...
Use `X-WNS-Type: wns/raw` with `Content-Type: application/octet-stream`. This delivers
raw bytes to the app's background task handler, which constructs and posts the
notification. The alternative `wns/toast` causes Windows to generate a notification from
an XML template before the app runs — avoid it.

WNS has no delivery priority field for raw notifications; urgency is carried in the
payload and the app's background task handler MAY use it to influence local notification
priority after delivery.

### Error responses

WNS reports errors in response headers, not the response body. Check on every response:
...
- `404 Not Found` / `410 Gone` — channel URI invalid; destroy the `PushSubscription`
- `406 Not Acceptable` / `channelthrottled` — rate limited; back off and retry
...
`200 OK`. For FCM, a `200` does not guarantee delivery — always check the response body.

| HTTP status | Meaning | Action |
|---|---|---|
| `200 OK` | Accepted (non-Web Push) | For FCM: check `error.status` in body. For others: none. |
| `201 Created` | Accepted (Web Push) | None |
| `400 Bad Request` | Malformed payload | Log and fix; do not retry this payload |
| `401 Unauthorized` | Credential expired | Refresh credential and retry |
| `403 Forbidden` | App unregistered or token invalid | Destroy the `PushSubscription` |
| `404 Not Found` | Endpoint gone | Destroy the `PushSubscription` |
| `410 Gone` | Subscription explicitly expired | Destroy the `PushSubscription` |
| `413 Payload Too Large` | Over size limit | Truncate and retry |
| `429 Too Many Requests` | Rate limited | Back off per `Retry-After`; consider falling back to `StateChange` |
| `5xx` | Push service error | Retry with exponential backoff |
...
expiry, not reused past their validity window. Static keys (MiPush App Secret) must be
stored as server-side secrets — never logged, never embedded in client code. APNs
token-based signing keys (`.p8` files) cannot be revoked through Apple's infrastructure;
treat them as long-lived secrets requiring equivalent protection to private CA keys.
